...
async function main(){
 ...
 try {
  ...
  if(args[0]==='init'){const c=await readConfig();const email=flag('--email')??c.email,code=flag('--code'),name=flag('--name')??'Mac Connector';if(c.connectorId&&c.token){out(true,'init',{stage:'ready',connectorId:c.connectorId});return;}const base=baseUrl(c);if(email){const unauth=new CompanionHttpClient({baseUrl:base});if(!code){const started=await unauth.emailStart(email);await writeConfig({...c,baseUrl:base,email});out(true,'init',{stage:'verification_required',email,mail:started});return;}const sessionToken=(await unauth.emailVerify(email,code)).sessionToken;const reg=await new CompanionHttpClient({baseUrl:base,token:sessionToken}).register(undefined,name);await writeConfig({...c,baseUrl:base,email,sessionToken,connectorId:reg.connectorId,token:reg.token});out(true,'init',{stage:'ready',connectorId:reg.connectorId});return;}if(!c.loginId||!c.pollSecret){const result=await new CompanionHttpClient({baseUrl:base}).loginStart();await writeConfig({...c,baseUrl:base,loginId:result.loginId,pollSecret:result.pollSecret});const {pollSecret:_,...safe}=result;out(true,'init',{stage:'browser_approval_required',...safe});return;}const result=await new CompanionHttpClient({baseUrl:base}).loginPoll(c.loginId,c.pollSecret);if(result.status==='pending'){out(true,'init',{stage:'browser_approval_pending',loginId:c.loginId});return;}await writeConfig({...c,baseUrl:base,loginId:undefined,pollSecret:undefined,connectorId:result.connectorId,token:result.token});out(true,'init',{stage:'ready',connectorId:result.connectorId});return;}
